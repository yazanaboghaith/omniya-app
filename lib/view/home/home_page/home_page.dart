...
class _HomePageState extends State<HomePage> {
          ...
          child: Column(
            children: [
              Text(
                ...
                style: AppTextStyles.text20Grey(
                  context,
                ),
              ),
              SizedBox(
  ...
  Widget _buildPrimaryPackageCard(
    ...
    final double totalQuota = (user?.baseService?.quota ?? 0).toDouble();

    final double remainingQuota =
        (user?.quota?.remainingDefault ?? 0).toDouble();

    final bool isUnlimited = user?.baseService?.unlimitted ?? false;

    final bool isExpired = user?.isExpired ?? false;

    double progressPercent = totalQuota > 0 ? remainingQuota / totalQuota : 0.0;
              ...
              Expanded(
                child: Container(
                  ...
                  child: Text(
                    ...
                    style: AppTextStyles.text10Grey(
                      context,
                    ),
                    maxLines: 1,
  ...
  Widget _buildAddonPackagesSection(
    double w,
    dynamic user,
  ) {
